...
export default function RunScanModal({ open, onOpenChange, onScanComplete }: RunScanModalProps) {
  ...
  const handleRunScan = async () => {
    try {
      setIsScanning(true);
      setError(null);
      const response = await outboundService.processScan();
    ...
    if (isScanning) {
      return (
        <div className="text-center py-8">
          ...
          <p className="text-lg font-medium mb-2">Processing outbound files...</p>
          <p className="text-sm text-muted-foreground">Scanning outbound files, please wait...</p>
        </div>
      );
    }
